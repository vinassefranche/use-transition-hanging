...
- see the button being stuck in `loading...` state

If the button is not stuck, try it a few times. If still not, re-start the application and try again. If not, add more `<BigBlock />` in the page as it seems like the more blocks we add, the more likely the issue is to occur
